...
impl Server {
    ...
    pub fn send_challenge(
        ...
        match self
        .vault
        .retrieve_public_key_for(username.as_str()) {
        ...
        match self
        .vault
        .retrieve_salt_for(username.as_str()) {
    ...
    pub fn ask_for_public_key(&self,username : &str) -> box_::PublicKey {
        match self
            .vault
            .retrieve_public_key_for(username)
        {
            Ok(pk) => return box_::PublicKey::from_slice(&decode(pk,Variant::UrlSafe).unwrap()).unwrap(),
    ...
    pub fn ask_for_metadata(&self,username : &str) -> &UserMetaData {
        match self
            .vault
            .retrieve_metadata_for(username)
        {
            Ok(m) => return m,
    ...
    pub fn ask_for_file(&self, b64_file_name_hash: &str) -> &UserEncryptedFile {
        match self
            .vault
            .retrieve_enc_file_by_b64_hash(b64_file_name_hash)
        {
            Ok(m) => return m,
